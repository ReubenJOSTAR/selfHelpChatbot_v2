...
# ── GENERATION SERVICE — old ecosystem compatible ──────────────
# Uses openai==1.25.2 (1.x SDK).
# AsyncOpenAI exists in openai>=1.0. This is correct.
# openai==0.28.x does NOT have AsyncOpenAI — that's why the
# requirements pin openai==1.25.2, not 0.28.1.
# ──────────────────────────────────────────────────────────────
from openai import AsyncOpenAI
...
class GenerationService:
    ...
    def __init__(self, settings: Settings):
        self._settings = settings
        # AsyncOpenAI — exists in openai>=1.0.0, including 1.25.2
        self._client = AsyncOpenAI(api_key=settings.OPENAI_API_KEY)
    ...
    async def generate(
        ...
        if not scriptures:
            return (
                "I wasn't able to find specific scripture for your situation. "
                "Please know that God sees you and cares deeply for you."
            )
        ...
        user_prompt = (
            f'A person came to you expressing: "{query}"\n\n'
            f"Relevant Scripture passages:\n\n{context}\n\n"
            f"Offer compassionate Christian guidance based only on these scriptures. "
            f"Close with a short prayer."
        )
